...
# Function to find bounding boxes and create YOLO labels
def find_bounding_box_and_create_yolo_label(image_path, output_path, class_id=0):
    ...
    if image_no_bg is None:
        print("Could not decode the image after background removal.")
        return None
    ...
    label_file = os.path.join(output_path, os.path.basename(image_path).replace(".jpg", ".txt"))
